...
describe('test', () => {
  ...
  it('checks if class [featuredContent] has been defined in styles', () => {
    ...
    splitCSSSolution.forEach(function(myCode, ind){
      if(myCode.indexOf("featuredContent")>0){
        expect(splitCSSSolution[ind+1].indexOf("background")).toBeGreaterThan(-1);
  ...
  it('checks if element [myContent] has been modified in JS', () => {
    expect(codeSolution.indexOf("classList.remove(\"normalContent\")")+codeSolution.indexOf("classList.remove(\'normalContent\')")).toBeGreaterThan(-1);
    expect(codeSolution.indexOf("classList.add(\"featuredContent\")")+codeSolution.indexOf("classList.add(\'featuredContent\')")).toBeGreaterThan(-1);
    expect(codeSolution.indexOf(".textContent")).toBeGreaterThan(-1);
    expect(window.test.innerHTML.toUpperCase().indexOf("FEATURED CONTENT")).toBeGreaterThan(-1);
    expect(window.window.test.classList.toString().replace(',',' ').indexOf("normalContent")).toBe(-1);
    expect(window.window.test.classList.toString().replace(',',' ').indexOf("featuredContent")).toBeGreaterThan(-1);
  });
});