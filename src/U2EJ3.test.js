...
describe('test', () => {
  ...
  beforeAll(() => {
    ...
    totalTitlesAfterFunction = window.document.getElementsByClassName("title").length; 
    totaParagraphsAfterFunction = window.document.getElementsByClassName("paragraph").length;
  });
  it('checks if function [createTitle] has been defined in code and is working', () => {
    expect(totalTitlesAfterFunction).toBe(1);
    expect(window.document.getElementsByTagName("h1")[0].innerHTML).toBe("titleX");
